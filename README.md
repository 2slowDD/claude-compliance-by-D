...
| `skills/d-handover` | Claude Code skill | Builds a copy/paste-ready handover prompt for a fresh agent — read-first sequence, F-* metrics, hard constraints, do-NOT list, closed-items ledger, environment preconditions, specific next action — with pre-emit state verification (P16), a live-background-work halt, a six-source FU census ordered for the receiving task, and ledger updates via `d-focus-tasks` before emitting |
