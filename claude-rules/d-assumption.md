...
- ⚠️ **That looser bar applies to CONVERSATIONAL tagging ONLY. It is overridden at the propagation boundary by [`verify-before-amplify`](verify-before-amplify.md) (P16):** once a load-bearing claim enters a **durable/shared artifact** (ledger, spec, plan, handover, memory — anything another agent will act on) or **gates a production action**, **🟢 requires a check *you* ran, with the check cited** (command / file:line / re-derivation). A source's *own* claim of verification — even "code-verified", even from a d-review, even from an earlier session of yours — is **⚠️ INHERITED**, never 🟢. **Unverified may sit in chat; it may not sit in the record.** *(This clause was added after the looser bar was found to license exactly the failure P16 exists to stop: an agent's "six code-verified defects" were republished into a ledger and a handover under a 🟢; four of the six were false and none had been checked.)*
