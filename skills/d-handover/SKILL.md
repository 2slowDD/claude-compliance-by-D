...
| `AI-Assets-Scanner` | cwd contains `WP-AI-Assets-Scanner` OR `AI-Assets-Scanner` as a path segment AND NOT `wpservice-saas` | `{AI_ROOT}\CU\AI Assets Scanner\WP-AI-Assets-Scanner` for the `WP-AI-Assets-Scanner` segment (the ACTIVE AAS repo from v1.9.0); `{AI_ROOT}\CU\AI Assets Scanner\AI-Assets-Scanner` for the legacy segment (closest ancestor matching the segment) | WP plugin; same ledger note. From v1.9.0 (operator 2026-09-26) all AAS work is in `WP-AI-Assets-Scanner` + PUBLIC remote `2slowDD/WP-AI-Assets-Scanner`; a cwd in the legacy `AI-Assets-Scanner` (≤ 1.8.9, read-only) should be flagged to the operator, not worked in. |
...
> - **P19 graded-decision-rubric — the fresh agent never hands the operator an open question.** On a complex choice (≥ 2 defensible approaches AND picking wrong costs rework / touches shipped behaviour / is awkward to reverse), it ranks the approaches itself against five ORDERED criteria — 1. F-\* impact by the F-priority order · 2. goal alignment against the ledger's active row (no ledger → the project's stated goal, named) · 3. new problems introduced · 4. reuse without breaking other callers (requires a grep) · 5. simplicity as tiebreak only — and leads with a recommendation carrying 🟢/🔴/🟡 per criterion plus **the criterion that decided it**. ⚠️ It does NOT relax "ask decisions plainly": operator-owned choices (customer-visible output, billing/credits, policy, irreversible, anything moving F-DEG) are still ASKED, explanation first — the rubric supplies the recommendation, never permission to skip the question. The tell it was skipped: a message ending "want me to…?" / "say the word" with no recommendation attached.
>
