...
- **`Step 7.0` → `Step 7.1`** — it was captioned "check this predicate FIRST" while numbered inside Step 7, so the caption and the numbering disagreed about execution order. The execution-sequence back-reference was updated to match.
- **⚠️ New required file: `skills/d-handover/references/docs-debt-prepass.md`.** Sub-sections 8.5.1–8.5.9 (~140 lines of docs-debt procedure) move there verbatim; Step 8.5 keeps its goal, its fire/skip predicate, the operator overrides, the operator-gated guarantee and the audit-footer shape. SKILL.md drops 624 → 500 lines, so detail needed only once the step fires now loads only then. **Existing installs must copy the new file** — the install block in README §5 has been updated.
- **`d-focus-tasks` "Trim / archive mechanics" absorbs the full D-Master-Ledger-Trim procedure** — fires threshold, what moves versus what the live file keeps, the backup-plus-conservation-asserts mechanism, and the Do-NOT list. The skill already owned the ledger; `claude-rules/d-master-ledger-trim.md` now defers to it instead of restating it.
- **`d-review` §5 no longer opens with a changelog of its own previous versions** — it named the exact output shape the section forbids, in the sentence introducing the prohibition. Same for the §5 colour-marker ruling; both now state the rule and its reason. The operative prohibition list is unchanged. Frontmatter `name:` `D-review` → `d-review`, matching the directory, CLAUDE.md P14 and the registered listing.

### Fixed — `skills/d-handover` (internal consistency + the handoff-doc render contract)

- **The skill violated its own machine-path invariant.** §2.0 forbids any hardcoded machine-absolute path; the Failure-Modes table named one machine's literal home, contradicting Step 1, which correctly uses `{CLAUDE_DIR}`. Now tokenised.
- **"No step is skippable" was false in its own document** — the pre-flight skips Steps 3, 4, 5, 5.5 and 10 on the no-ledger flag, and 8.5 skips on no closure signal or `--skip-docs-closure`. The line now names the six gates and holds the rule for everything else.
- **Dangling cross-reference** — the no-ledger pre-flight routed to a "Step 7.4" that does not exist; the target is intake Q4 inside Step 7.
- **Two acceptance criteria numbered 6**, so "AC 6" was ambiguous. Main list renumbered 1-7; conditional ACs namespaced C1-C6 so the two lists cannot collide again.
- **Anchor/citation mismatch** — the FU-ordering rule was defined as `5.6.` but cited twice as "Step 5.5.6"; it is now item 6 of Step 5.5.
- **Dated archaeology a single-form sweep had missed** — `caught <date>`, `(Changed <date>…)`, a trailing `— <date>.)` and `(Removed … PM per operator clarification)`, plus `(operator ruling <date>)` in `d-focus-tasks` ×2, a `Trip record <date>` opener in `d-review`, and two in `claude-rules/`. Swept with a widened pattern across all 157 skill/rule files: zero remaining.
- **New §9.3 documents the handoff-doc render contract.** `templates/handoff-doc.md` uses 17 placeholders; §9.1 named 5. Twelve slots of the load-bearing doc were filled by improvisation — including `{{HARD_CONSTRAINTS_DETAILED}}`, the render target for the Step 7.5 P12/P13/P16/P19 block, which nothing connected to its source.
...
- **A finding that states a count must survive a form-complete sweep (new §2a).** Searching a punctuated term (`optName:`) matches one syntactic form and is blind to shorthand forwarding and parameter declarations, so a threaded value undercounts silently — and the wrong number reads as authoritative. §2a requires the bare identifier, the raw hit count, and a partition of every hit (origination / declaration / forwarding / use); an empty cell where the mechanism requires a member means the sweep is wrong, not the code. Grounding: a review that published "four edit points" for a surface with ten, 2026-09-18.
...
- **Step 7.1 delegated intake** — when the operator delegates content ("ensure the next agent has all the needed details"), intake fills from session state with ONE batched confirmation instead of six sequential questions; everything auto-filled stays printed.
...
- **Docs-debt closure pre-pass** — completion phrasing (`Tasks N–M complete`) added to the signal table (now in `references/docs-debt-prepass.md`); the immediately-prior handoff doc always auto-candidates as HISTORICAL when writing a successor; mid-write candidates classify `AMBIGUOUS — mid-write` and defer past the live-work halt.
